...
import servicios from "../assets/service-8.png";
...
export const Services = ({ divRef }) => {
  ...
  const filteredServices = servicesInformation.filter((service) => {
  
    if (isDesktop && service.id === 8) {
      return false;
    }
  ...
  const getClasses = (i) => {
    switch (i) {
      case 0:
        return "md:col-start-1 md:col-end-3 md:row-start-2 md:row-end-5"; // Implante
      case 1:
        return "md:col-start-3 md:col-end-5 md:row-start-1 md:row-end-3"; // Limpieza
      case 2:
        return "md:col-start-3 md:col-end-5 md:row-start-3 md:row-end-5"; // Brackets
      case 3:
        return "md:col-start-5 md:col-end-9 md:row-start-1 md:row-end-5"; // Todos (Centro)
      case 4:
        return "md:col-start-9 md:col-end-11 md:row-start-1 md:row-end-3"; // Endodoncia
      case 5:
        return "md:col-start-9 md:col-end-11 md:row-start-3 md:row-end-5"; // Coronas/Blanqueamiento
      case 6:
        return "md:col-start-11 md:col-end-13 md:row-start-2 md:row-end-4"; // Cirugía
      // El case 7 (ID 8) ya no existe en desktop debido al filtro.
    }
  };
    ...
    >
      <div className="w-full flex justify-center px-4 items-center flex-col md:px-36">
        ...
        <section className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 lg:grid-rows-4 gap-4 mt-10 md:gap-2 md:max-w-[1150px]">
          {/* Usamos el array filtrado para renderizar las tarjetas */}
          {filteredServices.map((item, i) => (
            <CardService
              key={item.id}
              id={item.id} // Es importante pasar el ID para el renderizado condicional interno
